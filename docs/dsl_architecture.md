...
Важно не путать два похожих имени:

- `src/dsl/parsing` это синтаксическое ядро языка;
- `src/dsl/actors/parsing` это actor runtime для стадии разбора.

То есть это не два разных parsing-модуля в одном смысле. Один слой отвечает за сам разбор, второй за то, как этот разбор исполняется через coordinator, worker-подобные акторы и collector.
