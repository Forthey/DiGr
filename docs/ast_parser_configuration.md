...
- имя сущности
- дочернюю сущность или список дочерних сущностей, если они есть
- нужно ли явно строить `symbol` под этой сущностью
...
Если у сущности несколько типов дочерних узлов, можно передать список:
...
Строковая форма остаётся сокращением для списка из одного элемента:
...
Поле `symbols` на уровне entity явно управляет автоматическим созданием `symbol` под узлами этой сущности.

По умолчанию:

- для листовых сущностей `symbol` создаётся;
- для сущностей с `contains` `symbol` не создаётся.
...
semantic_block:
  ...
  symbols: true
  segmenter:
    kind: latex_semantic_block
```

Такой вариант полезен, когда у узла есть прикладные дочерние сущности, но DSL всё равно должен уметь считать расстояния в символах исходного текста внутри этого узла.

Если `symbols: false`, символы под сущностью не создаются даже для листа.
...
- что `contains` это строка или список строк
- что `symbols`, если указан, это boolean
